...
#else
static inline size_t zmk_animation_get_pixel_by_key_position(
    size_t key_position) {
    return key_position;
}
#endif
 ...
 * at a given distance (step) and store the resulting value in the given
 * pointer.
 *
...
static inline struct zmk_color_rgb zmk_apply_blending_mode(
    struct zmk_color_rgb base_value, struct zmk_color_rgb blend_value,
    uint8_t mode) {
